...
function hubspotMissingProperty(error, propertyName) {
  ...
  const fromErrors = errors.some(
    (e) =>
      e?.code === "PROPERTY_DOESNT_EXIST" &&
      Array.isArray(e?.context?.propertyName) &&
      e.context.propertyName.includes(propertyName),
  );
  if (fromErrors) return true;
  const text = String(data?.message || "");
  return text.includes(`Property "${propertyName}" does not exist`);
}
...
function buildEmployeePayload({ objectId, properties }) {
  ...
  return {
    ...
    stateProvince: normalizeStateProvince(properties?.home_state),
    phoneCell: normalizePhone(properties?.employee_phone),
  };
}

function buildBcbaMetadataPayload(properties) {
  return {
    email: cleanEmail(properties?.email),
...
function extractGenderDebug(properties) {
  const keys = Object.keys(properties || {}).filter((k) => /gender/i.test(k));
  const selectedKey = keys.find((k) => trimOrNull(properties?.[k])) || null;
  return {
    availableGenderKeys: keys,
    selectedGenderKey: selectedKey,
    selectedGenderValue: selectedKey ? trimOrNull(properties?.[selectedKey]) : null,
    includedInCrPayload: false,
  };
}
...
async function writebackHubspot({ hubspotToken, objectTypeId, objectId, properties, requestWithRetry }) {
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(properties, "employee_id") && hubspotMissingProperty(error, "employee_id")) {
      const fallbackProperties = { ...properties };
      delete fallbackProperties.employee_id;
      safeLog("bcba_writeback_employee_id_missing_on_object", {
        objectId: String(objectId),
        objectTypeId: String(objectTypeId),
        removedProperty: "employee_id",
      });
      await requestWithRetry(
        async () =>
          axios.patch(
        ...
        "hubspot.writeBack",
        {
          objectId: String(objectId),
          objectTypeId: String(objectTypeId),
          propertyKeys: Object.keys(fallbackProperties),
          fallbackWriteback: true,
        },
      );
      return;
    }
...
async function putMetadataField({ headers, contactId, fieldId, value, requestWithRetry, objectId }) {
  if (value == null || String(value).trim() === "") return;
  const body = metadataPutBody(value);
  logCrOutboundRequest({
    ...
    fieldId,
    method: "PUT",
    url: `${CR_BASE_URL}/contacts/${contactId}/metadata/${fieldId}`,
    payload: body,
  });
  await requestWithRetry(
    async () =>
      axios.put(`${CR_BASE_URL}/contacts/${contactId}/metadata/${fieldId}`, body, {
        headers,
        timeout: 30000,
      }),
    "cr.metadata.updateField",
    { objectId: String(objectId), contactId: String(contactId), fieldId },
  );
}

async function putBcbaMetadataFields({ headers, contactId, metadataValues, requestWithRetry, objectId }) {
  await putMetadataField({
    headers,
    contactId,
    fieldId: BCBA_METADATA_FIELD_IDS.email,
    value: metadataValues.email,
    requestWithRetry,
    objectId,
  });
  await putMetadataField({
    headers,
    contactId,
    fieldId: BCBA_METADATA_FIELD_IDS.medicaid_id__ny,
    value: metadataValues.medicaid_id__ny,
    requestWithRetry,
    objectId,
  });
  await putMetadataField({
    headers,
    contactId,
    fieldId: BCBA_METADATA_FIELD_IDS.medicaid_id__nj,
    value: metadataValues.medicaid_id__nj,
    requestWithRetry,
    objectId,
  });
  await putMetadataField({
    headers,
    contactId,
    fieldId: BCBA_METADATA_FIELD_IDS.medicaid_id__co,
    value: metadataValues.medicaid_id__co,
    requestWithRetry,
    objectId,
  });
}
...
async function processOneRecord({
  ...
  if (contactId) {
    ...
    safeLog("bcba_contactid_identifier_preserve", {
      objectId: String(objectId),
...
export default defineComponent({
  name: "HS->CR Employee Push (Step 2 BCBA)",
  description: "Processes BCBA custom object IDs from Step 1 and syncs to CentralReach employees.",
  version: "1.0.0",
  props: {
  ...
  async run({ steps }) {
    ...
    for (const objectId of objectIds) {
      try {
        ...
        safeLog("bcba_step2_record_failed", {
          objectId: String(objectId),
          ...meta,
        });
        ...
        try {
          ...
          safeLog("bcba_step2_error_writeback_failed", {
            objectId: String(objectId),
